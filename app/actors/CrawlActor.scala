...
class CrawlStatisticsActor extends Actor {
  ...
  def statsHtml = {
    ...
    "<pre>" +
      ...
      ("last period: total %.2f 1/s, %.2f kBs, avg response %.2fms, avg page size %.2fkB\n" format (crawlsInPeriod/fromLast, bytesInPeriod/fromLast/1024.0, if(crawlsInPeriod!=0) durationInPeriod/crawlsInPeriod/1000000.0 else 0.0), if (crawlsInPeriod!=0) 1.0*bytesInPeriod/crawlsInPeriod) +
    "</pre>"
  }
